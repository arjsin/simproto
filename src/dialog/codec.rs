use bytes::{BytesMut, BufMut, LittleEndian, ByteOrder};
use tokio_io::codec::{Encoder, Decoder};
use dialog::frame::{Frame, TypeLabel};
use std::{io, mem};
...
impl Encoder for Codec {
    ...
    fn encode(&mut self, frame: Self::Item, buf: &mut BytesMut) -> io::Result<()> {
        let payload_len = frame.message_len();
        let len = payload_len as usize + 5;
        let id = frame.id();
        buf.reserve(len);
        buf.put_u8(frame.frame_type().into());
        buf.put_u64_le(payload_len as u64);
        buf.put_u64_le(id);
        buf.extend_from_slice(&frame.into_message());
        Ok(())
    }
}
...
mod test {
    ...
    const ENCODED: [u8; 21] = [1, 4, 0, 0, 0, 0, 0, 0, 0, 12, 0, 0, 0, 0, 0, 0, 0, 1, 2, 3, 4];
    #[test]
    fn encode() {
